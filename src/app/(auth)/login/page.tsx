...
export default function LoginPage() {
  ...
  return (
    <div className="grid grid-cols-2 min-h-screen bg-cover bg-center">
      {/* Left panel */}
      <div className="flex min-h-screen relative items-center justify-center mr-5">
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen w-full px-10 gap-8">
          <img
            src="/images/logo_white.png"
            alt="Finotiq Logo"
            className="absolute top-12 left-8 bot-10 z-30 h-80 w-auto"
          />
      ...
      {/* Right panel */}
      <div className="w-[30%] h-9/10 bg-white rounded-t-2xl flex items-center justify-center px-6 absolute bottom-0 left-[50%] ml-10">
        <div className="w-full max-w-md space-y-10">
          <div className="text-center space-y-4">
            <h1 className="text-4xl font-bold text-[#003c45]">Đăng nhập</h1>
            <p className="text-[#545454]">Dùng tài khoản đã đăng ký để đăng nhập vào hệ thống</p>
          </div>
          {error && (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-2 mb-7">
              <Label htmlFor="email" className="text-[#003c45] mb-3">
                Email:
              </Label>
              <Input
                id="email"
            ...
            <div className="space-y-2 mb-7">
              <Label htmlFor="password" className="text-[#003c45] mb-3">
                Mật khẩu:
              </Label>
              <Input
                id="password"
            ...
            <Button
              className="w-full bg-[#003C45] hover:bg-[#00262c] text-[#F4FAB9] font-bold border-none"
              type="submit"
              disabled={loading}
            >
              {loading ? (
                ...
                <span className="text-[#F4FAB9] cursor-pointer">ĐĂNG NHẬP</span>
              )}
            </Button>
          </form>
